...
├── llm_input.json       # Deterministic article brief for LLM
...
2. Copy the `SUMMARY` block into front matter and `ARTICLE_BODY` into `jekyll_draft.md`
...
* **Structured article brief** ensures repeatability and keeps chronology, priority topics and evidence explicit
...
8. Build a richer article brief with meeting overview, priority topics and structured evidence
...
* Structured LLM prompts with meeting overview, priority topics and topic evidence
...
`llm_input.json` now contains a deterministic article brief instead of a thin topic list. It includes:

* `meeting_overview`: total duration, number of included topics, longest topics, procedural share, discussion share, meeting character
* `priority_topics`: the strongest topics for the article lead
* `topics`: chronological topic entries with timing, speaker signals, keywords, summary hints and structured evidence
