...
* *Structured LLM prompts*: Article-focused prompts with meeting overview, priority topics and topic evidence
...
* `meeting_overview` with total duration and meeting-level signals
* `priority_topics` for the article lead
* Chronological `topics` with timing, topic type, keywords and structured evidence
...
* Clear article-writing objective and reader-value guidance
* Meeting overview and priority topics
* Topic data with structured evidence
* Explicit `SUMMARY` and `ARTICLE_BODY` output contract
...
Writing Objective:
- napiš středně dlouhý článek ...

Reader Value:
- ukaž konkrétní rozhodnutí, spory, praktické dopady ...
...
5. **Copy** `SUMMARY` into the front matter and `ARTICLE_BODY` into `jekyll_draft.md`
...
The article generator now expects a deterministic article brief instead of just a bare topic list. Each topic can include:

* `time_range`, `start_minute`, `end_minute`
* `speaker_count` and `dominant_speaker_ratio`
* `keywords`
* `segments_count`
* `discussion_intensity`
* `topic_summary_hint`
* `evidence` objects with `text`, `time_range`, `start_minute`, `end_minute`, `evidence_type`
...
{
  "meeting_overview": {
    "total_meeting_minutes": 241.0,
    "included_topic_count": 6,
  ...
  "topics": [
    {
      "order": 2,
      "time_minutes": 74.0,
      "time_range": "36.0–110.0 min",
      "speaker_count": 6,
      "keywords": ["stavba", "silnice", "Olomoucká"],
      "discussion_intensity": "high",
      "topic_summary_hint": "průběh stavby...; typy zmínek: concern, decision",
      "evidence": [
        {
          "text": "Nejdelší debata se vedla o rekonstrukci Olomoucké ulice...",
          "time_range": "36.0–41.0 min",
